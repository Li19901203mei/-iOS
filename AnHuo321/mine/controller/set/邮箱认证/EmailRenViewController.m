...
@interface EmailRenViewController ()
...
@implementation EmailRenViewController
...
- (void)viewDidLoad {
    ...
    self.nameField.placeholder = @"输入你的邮箱";
    ...
    [[maBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        [HttpTool post:KURLNSString(sendCode) dic:@{@"checktp":@"email",@"how":@"verify",@"checkadr":self.nameField.text} success:^(id  _Nonnull responce) {
    ...
    [[sureBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        if (self.typeIndex==1) {
            ...
            [HttpTool post:KURLNSString(checkCode) dic:@{@"checkType":@"email",@"email_verify":self.yanField.text,@"email":self.nameField.text} success:^(id  _Nonnull responce) {
                if ([responce[@"result"] intValue]==1) {
                    [SVProgressHUD showWithStatus:[NSString stringWithFormat:@"%@",responce[@"data"]]];
