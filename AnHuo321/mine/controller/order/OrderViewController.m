...
- (void)setupview{
    ...
    //买家
    bgview1 = [AHControll viewWithFrame:CGRectMake(10, 10, SCREEN_WIDTH-20, 310) viewColor:UIColor.whiteColor];
    LRViewBorderRadius(bgview1, 10, 0, UIColor.clearColor);
    [scrollview addSubview:bgview1];
    UIView *vLine1 = [AHControll viewWithFrame:CGRectMake(0, 39, SCREEN_WIDTH-20, 1) viewColor:COLOR];
    ...
    UIButton *moreBtn1 = [AHControll createButtonNormalFrame:CGRectMake(SCREEN_WIDTH-100, 10, 80, 20) Target:self Action:@selector(clickMoreOne) Title:@"查看全部" TitleColor:UIColor.darkGrayColor Font:14 BackGColor:UIColor.whiteColor];
    [bgview1 addSubview:moreBtn1];
    
    
    //卖家
    bgview2 = [AHControll viewWithFrame:CGRectMake(10, 340, SCREEN_WIDTH-20, 310) viewColor:UIColor.whiteColor];
    LRViewBorderRadius(bgview2, 10, 0, UIColor.clearColor);
    [scrollview addSubview:bgview2];
    UIView *vLine2 = [AHControll viewWithFrame:CGRectMake(0, 39, SCREEN_WIDTH-20, 1) viewColor:COLOR];
    ...
    UIButton *moreBtn2 = [AHControll createButtonNormalFrame:CGRectMake(SCREEN_WIDTH-100, 10, 80, 20) Target:self Action:@selector(clickMoreTwo) Title:@"查看全部" TitleColor:UIColor.darkGrayColor Font:14 BackGColor:UIColor.whiteColor];
    ...
    UIView *bg1 = [AHControll viewWithFrame:CGRectMake(10, 240, SCREEN_WIDTH-40, 60) viewColor:COLOR];
    LRViewBorderRadius(bg1, 5, 0, UIColor.clearColor);
    [bgview1 addSubview:bg1];
    
    UIView *bg2 = [AHControll viewWithFrame:CGRectMake(10, 240, SCREEN_WIDTH-40, 60) viewColor:COLOR];
...
- (void)setButtonOne{
    NSMutableArray *arr1 = [@[@"待付款",@"待发货",@"待收货",@"交易成功",@"已评价",@"已互评",@"退货/售后"]mutableCopy];
    NSMutableArray *arr2 = [@[@"a1",@"a2",@"a3",@"a4",@"a4",@"a4",@"a1"]mutableCopy];
    int b = 0;
    int hangshu;
    if (arr1.count % 5 == 0 ) {
...
- (void)clickBtnOne:(UIButton *)sender{
    ...
    if ([sender.titleLabel.text isEqualToString:@"待付款"]) {
        ...
        vc.status = @"0";
        [self.navigationController pushViewController:vc animated:YES];
    }else if ([sender.titleLabel.text isEqualToString:@"待发货"]) {
        BuyOrderViewController *vc = [[BuyOrderViewController alloc]init];
        vc.title = sender.titleLabel.text;
        vc.status = @"1";
        [self.navigationController pushViewController:vc animated:YES];
        ...
        vc.status = @"2";
        [self.navigationController pushViewController:vc animated:YES];
    }else if ([sender.titleLabel.text isEqualToString:@"交易成功"]) {
        BuyOrderViewController *vc = [[BuyOrderViewController alloc]init];
        vc.title = sender.titleLabel.text;
        vc.status = @"3";
        [self.navigationController pushViewController:vc animated:YES];
    }else if ([sender.titleLabel.text isEqualToString:@"已评价"]) {
        BuyOrderViewController *vc = [[BuyOrderViewController alloc]init];
        vc.title = sender.titleLabel.text;
        vc.status = @"5";
        [self.navigationController pushViewController:vc animated:YES];
    }else if ([sender.titleLabel.text isEqualToString:@"已互评"]) {
        BuyOrderViewController *vc = [[BuyOrderViewController alloc]init];
        vc.title = sender.titleLabel.text;
        vc.status = @"11";
        [self.navigationController pushViewController:vc animated:YES];
    }else{
        SalesReturnVC *vc = [SalesReturnVC new];
        vc.title = sender.titleLabel.text;
        [self.navigationController pushViewController:vc animated:YES];
    }
